...
exports.loadMatchesForProfile = async function(req, res) {
  try {
    let profileId = req.params.profileId;
    let query = `SELECT l.id as like_id , l.like_back , l.dislike_back , p.username AS username, p.id AS profile_id, p.profile_pic AS profile_pic FROM likes AS l, users AS p WHERE l.user_id = p.id AND l.profile_id  = ${profileId} AND l.like_back = 1 AND l.dislike_back = 0`;
